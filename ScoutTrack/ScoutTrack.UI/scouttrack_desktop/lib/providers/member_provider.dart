...
class MemberProvider extends BaseProvider<Member, dynamic> {
  ...
  Future<Member> updateProfilePicture(int id, File? imageFile) async {
    final uri = Uri.parse("${BaseProvider.baseUrl ?? "http://localhost:5164/"}$endpoint/$id/update-profile-picture");
    ...
    if (imageFile == null) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final error = jsonDecode(response.body);
          throw Exception(error['title'] ?? 'Greška prilikom brisanja profilne fotografije.');
        }
      } catch (e) {
